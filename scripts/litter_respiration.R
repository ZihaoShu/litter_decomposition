...
paths <- stringr::str_c("D:/shuzihao/litter_decomposition/output/Insuit/plots/litter_respiration/first_",
...
CO2_evolution <- read.csv("output/Insuit/first_respiration_data.csv") %>% 
  ...
  rename(k = Coef,mass = mean) %>% 
  mutate(CO2_evolution = (k*44*0.5*1.013*273.15)/(mass*22.4*1.013*(273.15+Temperature)))
write.csv(CO2_evolution,"output/Insuit/CO2_evolution.csv")

CO2_evolution$Group <- factor(CO2_evolution$Group,levels = c("CK","N1","N2","P1","P2","NP1","NP2"))
p_CO2_evolution <- ggplot(CO2_evolution,aes(x = Group,y = CO2_evolution))+
  geom_point(size = 4)+
  facet_grid(Type ~ .)+
  labs(x = " ",
       y = expression(CO[2]~evolution~rate~(μg~C~g^-1~dry~mass~min^-1)))+
  theme_szh()
ggsave("D:/shuzihao/litter_decomposition/output/Insuit/plots/CO2_evolution.png",
